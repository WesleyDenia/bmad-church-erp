...
use App\Domain\Finance\Services\ListFinancialPendingItemsService;
use App\Http\Resources\FinancialPendingItemListResource;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Gate;

class ListFinancialPendingItemsController
{
    public function __invoke(
        Request $request,
        ListFinancialPendingItemsService $service,
    ): JsonResponse {
        $user = $request->user();
        ...
        return (new FinancialPendingItemListResource($service->list($churchId)))
            ->response()
            ->setStatusCode(200);
    }
