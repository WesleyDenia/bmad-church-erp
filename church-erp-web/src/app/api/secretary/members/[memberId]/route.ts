...
function buildInvalidMemberIdResponse(): Response {
  return NextResponse.json(
    {
      message: "Revise a identificacao do membro e tente novamente.",
      errors: {
        member_id: ["Informe um membro valido."],
      },
    },
    { status: 422 },
  );
...
function pickMemberPatchPayload(body: Record<string, unknown>): Partial<MemberPayload> | null {
  ...
  for (const field of MEMBER_PAYLOAD_ALLOWLIST) {
    ...
    if (field === "status") {
      payload.status = value as MemberPayload["status"];
      continue;
    }
...
function buildSafeBody(status: number, body: Record<string, unknown>): MemberErrorResponse {
  ...
  return {
    message: "Nao foi possivel carregar o membro agora.",
  };
}

async function resolveMemberId(params: RouteContext["params"]): Promise<string | null> {
  const { memberId } = await params;

  return isSafePositiveIntegerId(memberId) ? memberId : null;
}
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  const memberId = await resolveMemberId(context.params);

  if (memberId === null) {
    return buildInvalidMemberIdResponse();
  }
  ...
  const response = await callLaravel(`/api/v1/people/members/${memberId}`, {
    method: "GET",
    cache: "no-store",
    headers: {
...
export async function PATCH(request: Request, context: RouteContext): Promise<Response> {
  ...
  const payload = pickMemberPatchPayload(requestBody);
