...
function isSafePositiveIntegerId(value: string): boolean {
  if (!/^[1-9]\d*$/.test(value)) {
    return false;
  }
...
function buildInvalidVisitorIdResponse(): Response {
  return NextResponse.json(
    {
      message: "Revise a identificacao do visitante e tente novamente.",
      errors: {
        visitor_id: ["Informe um visitante valido."],
      },
    },
    { status: 422 },
  );
...
function pickVisitorPatchPayload(body: Record<string, unknown>): Partial<VisitorPayload> | null {
  ...
  const payload: Partial<VisitorPayload> = {};
  ...
  for (const field of VISITOR_PAYLOAD_ALLOWLIST) {
    ...
    if (field === "status") {
      payload.status = value as VisitorPayload["status"];
      continue;
    }

    if (field === "display_name") {
      payload.display_name = typeof value === "string" ? value : "";
      continue;
    }

    if (field === "phone") {
      payload.phone = typeof value === "string" || value === null ? value : "";
      continue;
    }

    payload.email = typeof value === "string" || value === null ? value : "";
  }
...
function buildSafeBody(status: number, body: Record<string, unknown>): VisitorErrorResponse {
  ...
  return {
    message: "Nao foi possivel carregar o visitante agora.",
  };
}

async function resolveVisitorId(params: RouteContext["params"]): Promise<string | null> {
  const { visitorId } = await params;

  return isSafePositiveIntegerId(visitorId) ? visitorId : null;
}
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  const visitorId = await resolveVisitorId(context.params);

  if (visitorId === null) {
    return buildInvalidVisitorIdResponse();
  }
  ...
  const response = await callLaravel(`/api/v1/people/visitors/${visitorId}`, {
    method: "GET",
    cache: "no-store",
    headers: {
...
export async function PATCH(request: Request, context: RouteContext): Promise<Response> {
  ...
  const payload = pickVisitorPatchPayload(requestBody);
